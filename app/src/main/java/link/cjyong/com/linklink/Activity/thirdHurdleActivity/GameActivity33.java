...
/**
 * Created by cjyong on 2017/4/12.
 * 第三大关第三小关游戏Activity类
 */

public class GameActivity33 extends Activity
{
    //道具的按钮
    private Button pauseBtn,helpBtn,resetBtn,timeBtn,boomBtn;
    //任务需要消除的Image
    ...
    private void init()
    {
        ...
        taskImage.setImageBitmap(ImageUtil.getSpecialPieceSignImage());
        ...
        successDialog = createDialog("Success", "游戏胜利！ 重新开始",
                ...
                .setNegativeButton("返回",new DialogInterface.OnClickListener()
                {
                    public void onClick(DialogInterface dialog, int which)
                    {
                        Intent  intent = new Intent(GameActivity33.this,ThirdHurdleActivity.class);
        ...
        soundBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(MyApplication.getBackIsOn()==1)
                {
                    ...
                    soundBack.setImageResource(R.drawable.soundback2);
                    //发送暂停信息
                    Intent intent = new Intent(GameActivity33.this,BackgroundMusicService.class);
        ...
        settingAD = new AlertDialog.Builder(this)
                ...
                .setNegativeButton("返回", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        //切换背景音乐
                        /*
                        Intent tintent = new Intent(GameActivity33.this,BackgroundMusicService.class);
    ...
    private void handleSuccessLink(LinkInfo linkInfo, Piece prePiece, Piece currentPiece, Piece[][] pieces) {
        ...
        if(prePiece.isSpecial())
        {
            if(prePiece.getClickedNum()>=1) {
                prePiece.setImage(ImageUtil.getRandomPieceImage());
                prePiece.setClickedNum(prePiece.getClickedNum() - 1);
            }
            else
            {
                step--;
                timesLeft.setText(step+"");
                prePiece.setSpecial(false);
            }
        }

        if(currentPiece.isSpecial())
        {
            if(currentPiece.getClickedNum()>=1) {
                currentPiece.setImage(ImageUtil.getRandomPieceImage());
                currentPiece.setClickedNum(currentPiece.getClickedNum() - 1);
            }
            else
            {
                step--;
                timesLeft.setText(step+"");
                currentPiece.setSpecial(false);
            }
        }
        ...
        this.gameView.postInvalidate();
        this.gameService.downMoveSpecialPiecesAndFilling(prePiece,currentPiece);
        this.selected = null;
        if (step<=0) {
            //游戏胜利
            success(useStep);
    ...
    private void startGame(int step) {
        ...
        if (step == config.getGameTime()) {
            gameView.startGame();
            gameService.addSpecialPieces(config.getGameTime());
            timesLeft.setText(step+"");
        }
        isPlaying = true;
        this.selected = null;
